...
void showMenu(const FirmwareSlot* slots, int count, int selected, int countdownMs) {
    ...
    // Menu items: 11px rows starting at y=15 so all 4 slots clear the
    // 3px countdown bar at the bottom of the 64px-tall screen.
    const int itemH = 11;
    const int startY = 15;
    ...
    for (int i = 0; i < count && i < MAX_SLOTS; i++) {
        ...
        // Slot name comes from sel_cfg (written by the Python flasher from
        // the .bin filename), resolved into slots[i].name by FirmwareManager::scan.
        u8g2.setFont(u8g2_font_helvR08_tr);
        char label[64];
        const char* prefix = (i == selected) ? ">" : " ";
        const char* suffix = slots[i].valid ? "" : " (empty)";
        snprintf(label, sizeof(label), "%s %d. %s%s",
                 prefix, i + 1, slots[i].name, suffix);
